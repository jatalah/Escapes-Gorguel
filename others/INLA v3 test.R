...
# border <- as.data.frame(murcia_coast[[1]][[1]][[1]]) %>%
#   st_as_sf(coords = c("V1", "V2"), crs = 4326, agr = "constant")
border_poly <- as_Spatial(murcia_coast)
...
SST_medi <- SST_medi_2
esc_d <- read_csv('data/gorguel_density_data_clean_depth.csv')

# Variograma
  density_geo <- data.frame(lon=esc_d$X,lat=esc_d$Y,data=esc_d$density)
  coords <- density_geo[,c("lon","lat")]
  
  density_geo1 <- as.geodata(density_geo)
  
  # Eliminacion de tendencias
  SST_ls<-with(density_geo, surf.ls(3,lon,lat,density))
  ...
  coordinates(esc_d) = ~X+Y
  variogram(log(density+1)~1, esc_d)
...
# Mesh
  ...
  ggplot() +
    geom_sf(data = murcia_coast, fill = "lightblue", col = "transparent")+
